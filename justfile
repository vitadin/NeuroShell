...
default:
    ...
    @echo "  build-fast        - Fast development build (preserves Go cache, full linting)"
    @echo "  build-dev         - Ultra-fast build (skip validation - use sparingly!)"
    @echo "  build-parallel    - Build both binaries simultaneously"
    ...
    @echo "  clean             - Clean build artifacts and Go cache (full clean)"
    @echo "  clean-artifacts   - Clean only build artifacts (preserves Go cache)"
    @echo "  format            - Format Go code and organize imports"
    @echo "  format-quick      - Quick format (skip import organization)"
